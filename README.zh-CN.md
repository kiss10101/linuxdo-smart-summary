...
<p align="center">
  ...
  <a href="https://github.com/kiss10101/linuxdo-smart-summary/releases/download/v7.8.0-alpha.3/linuxdo-smart-summary-7.8.0-alpha.3.user.js">预览版 7.8.0-alpha.3</a> ·
...
| 预览版 | `7.8.0-alpha.3` | 需要 beta 前最后一版交互预览 | 将仅限最终答案的选区工具栏扩展到完成态 AI 回复，增加可调悬浮菜单不透明度，压缩消息菜单，并统一总结/对话阅读滚动条到左侧 |
...
`7.8.0-alpha.3` 是当前 prerelease 预览版。`alpha.3` 是经用户明确追加批准、位于
模块化和默认 UI alpha 之后的最后功能门禁；此后 `beta.1` 只处理兼容性和缺陷收敛，
...
- 推送新标签，例如 `v7.8.0-alpha.3`，会自动触发发版。
- 对已经存在的标签，可以在 GitHub Actions 页面手动运行 `Release` workflow，输入 `7.8.0-alpha.3` 来发布或修复该版本的 Release。
