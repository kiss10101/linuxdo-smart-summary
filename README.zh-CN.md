...
<p align="center">
  ...
  <a href="https://github.com/kiss10101/linuxdo-smart-summary/releases/download/v7.8.0-alpha.4/linuxdo-smart-summary-7.8.0-alpha.4.user.js">预览版 7.8.0-alpha.4</a> ·
...
| 预览版 | `7.8.0-alpha.4` | 需要 beta 前的 SPA 工作区生命周期修复 | 站内主题间直接切换时保留当前总结与对话，标明来源主题，跨主题替换前确认，并让生成中的工作继续归属其来源主题 |
...
`7.8.0-alpha.4` 是当前 prerelease 预览版。`alpha.4` 是经用户明确批准、位于最终
功能门禁 `alpha.3` 之后的一次有边界的生命周期修复例外；此后 `beta.1` 只处理兼容性和缺陷收敛，
`7.8.0` 用于稳定版晋级，不再规划新的功能 alpha。历史 `7.7` 条目保留在 CHANGELOG 中用于审计，
不再算作迁移发布站点。
...
- 推送新标签，例如 `v7.8.0-alpha.4`，会自动触发发版。
- 对已经存在的标签，可以在 GitHub Actions 页面手动运行 `Release` workflow，输入 `7.8.0-alpha.4` 来发布或修复该版本的 Release。
