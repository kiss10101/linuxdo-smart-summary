...
<p align="center">
  ...
  <a href="https://github.com/kiss10101/linuxdo-smart-summary/releases/download/v7.8.0-beta.2/linuxdo-smart-summary-7.8.0-beta.2.user.js">预览版 7.8.0-beta.2</a> ·
...
| 预览版 | `7.8.0-beta.2` | 需要完整 7.8 功能和实时 Markdown 流式渲染 | 修复生成期间暴露原始 Markdown 标记的问题，同时保留自适应渲染合并、资源预算和浏览器生命周期加固 |
...
`7.8.0-beta.2` 是当前 prerelease 预览版。`beta.2` 是一次有边界的回归修复，
恢复实时 Markdown 渲染但不重新开放功能范围；下一计划版本仍是稳定晋级版 `7.8.0`，
不再规划新的 prerelease 功能。历史 `7.7` 条目保留在 CHANGELOG 中用于审计，
不再算作迁移发布站点。
...
| 运行时调度 | 每个合并后的流式更新都会实时解析并清理最终答案 Markdown；长输出按 `80 / 140 / 220ms` 自适应降低频率，流式推理继续使用转义纯文本，并合并滚动/尺寸任务 |
| 主题页启动 | 只在 Linux.do 主题路由创建侧栏，非主题页保留轻量 SPA 路由监听 |
| SPA 工作区生命周期 | 主题间直接切换时保留同一个内存总结/对话工作区，标明来源主题、确认跨主题替换，并在离开主题路由时销毁 |
| AI 重试路径 | AI 空回或报错后，复用当前标签页内刚获取的总结文本 |
| 总结后对话 | UI 元数据和服务返回的推理不会进入后续 API 请求；消息操作会随完成、生成中、已停止和失败状态变化，并对当前回复的停止、重新生成和删除做过期请求防护 |
...
| 最终答案选区 | 总结最终答案或完成态 AI 回复中的选中文本提供“解释、精简、引用到对话”；推理、状态和来源元数据位于可选择答案边界之外，引用会保留当前草稿且永不自动发送 |
| 悬浮菜单 | 选区与消息操作菜单使用可调 `80%`–`100%` 不透明表面（默认 `88%`），不改变模态框、toast、tooltip 或侧栏透明度 |
| 阅读滚动条 | 总结与对话阅读滚动条通过局部滚动壳保持在左侧，答案正文、代码、表格和横向滚动仍保持正常方向 |
| 默认 UI | Style2 采用克制的暖中性色与钢蓝色系统，补齐键盘操作、减少动画和窄屏覆盖回退；Style1 继续作为兼容主题 |
...
| 导出 | 将明确拉取到的帖子集合导出为 HTML 或 AI 可读文本；离线图片嵌入支持取消，并受单图、总编码体积和超时限制 |
...
- 推送新标签，例如 `v7.8.0-beta.2`，会自动触发发版。
- 对已经存在的标签，可以在 GitHub Actions 页面手动运行 `Release` workflow，输入 `7.8.0-beta.2` 来发布或修复该版本的 Release。
