...
<p align="center">
  ...
  <a href="https://github.com/kiss10101/linuxdo-smart-summary/releases/download/v7.8.0-alpha.2/linuxdo-smart-summary-7.8.0-alpha.2.user.js">预览版 7.8.0-alpha.2</a> ·
...
| 预览版 | `7.8.0-alpha.2` | 需要交互与默认 UI 预览版 | 保留模块化源码与单文件安装，精简选区操作，新增状态化消息控制，并重设计默认 Style2 及窄屏回退 |
...
`7.8.0-alpha.2` 是当前 prerelease 预览版。`alpha.2` 是模块化迁移后一次性加入的
功能与默认 UI 门禁；此后 `beta.1` 只处理兼容性和缺陷收敛，`7.8.0` 用于稳定版
晋级，不再规划新的功能 alpha。历史 `7.7` 条目保留在 CHANGELOG 中用于审计，
不再算作迁移发布站点。
...
| 总结选区 | 选中的总结文本提供“解释、精简、引用到对话”三个操作；引用会保留当前草稿且永不自动发送 |
...
- 推送新标签，例如 `v7.8.0-alpha.2`，会自动触发发版。
- 对已经存在的标签，可以在 GitHub Actions 页面手动运行 `Release` workflow，输入 `7.8.0-alpha.2` 来发布或修复该版本的 Release。
