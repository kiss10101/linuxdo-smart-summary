...
<p align="center">
  ...
  <a href="https://github.com/kiss10101/linuxdo-smart-summary/releases/download/v7.8.0-beta.1/linuxdo-smart-summary-7.8.0-beta.1.user.js">预览版 7.8.0-beta.1</a> ·
...
| 预览版 | `7.8.0-beta.1` | 需要完整 7.8 功能并完成兼容性与资源开销收敛 | 保留 alpha 功能集，同时减少流式渲染开销、限制临时缓存与离线图片导出，并加固浏览器生命周期 |
...
`7.8.0-beta.1` 是当前 prerelease 预览版。该 beta 在已完成的 alpha 功能线之后，
只收敛兼容性、缺陷和资源开销风险；下一计划版本为稳定晋级版 `7.8.0`，不再规划新的
prerelease 功能。历史 `7.7` 条目保留在 CHANGELOG 中用于审计，
不再算作迁移发布站点。
...
| 运行时调度 | 流式阶段显示转义纯文本预览，完成时只渲染一次 Markdown；长输出自适应降低更新频率，并合并高频滚动/尺寸任务 |
...
- 推送新标签，例如 `v7.8.0-beta.1`，会自动触发发版。
- 对已经存在的标签，可以在 GitHub Actions 页面手动运行 `Release` workflow，输入 `7.8.0-beta.1` 来发布或修复该版本的 Release。
