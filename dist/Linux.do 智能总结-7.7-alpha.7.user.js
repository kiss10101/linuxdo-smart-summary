...
(function() {
    ...
    const Core = {
        ...
        classifyAiOutput(rawText, meta = {}) {
            ...
            if (content) {
                return { kind: 'success', rawText: raw, thinking, content, finishReason };
            }
            if (finishReason === 'content_filter') {
                return { kind: 'finish_content_filter', rawText: raw, thinking, content: '', finishReason };
            }
            if (finishReason === 'length') {
                return { kind: 'finish_length', rawText: raw, thinking, content: '', finishReason };
            }
            if (finishReason === 'tool_calls' || finishReason === 'function_call') {
                return { kind: 'finish_unsupported', rawText: raw, thinking, content: '', finishReason };
            }
            if (thinking) {
                return { kind: 'thinking_only', rawText: raw, thinking, content: '', finishReason };
            }
            return { kind: 'empty_response', rawText: raw, thinking: '', content: '', finishReason };
        },
        ...
        createModelOutputFailure(classified, options = {}) {
            ...
            const map = {
                thinking_only: ['empty', 'AI 只返回了思考过程', '没有生成可展示的正文。可重新生成或更换模型。'],
    ...
    UIRegistry.register('style1', {
        ...
        * { box-sizing: border-box; }
            ...
            try {
                await Core.streamChat(messages,
                    ...
                    (meta = {}) => {
                        ...
                        const classified = Core.classifyAiOutput(aiText, meta);
        ...
        renderBubbleContent(div, message) {
            ...
            div.innerHTML = this.renderWithThinking(message.rawContent || message.content, message.status === 'streaming') + source;
        },
                ...
                await Core.streamChat(messages,
                    ...
                    (meta = {}) => {
                        ...
                        if (classified.kind !== 'success') {
                            ...
                            resultBox.innerHTML = `
                                ${Core.renderAiFailureBlock(failure, { operation: 'summary' })}
                        ...
                        this.clearAiAbortController(abortController);
                        this.setLoading('#btn-summary', false);
                        this.updateResultBox(resultBox, aiText, false, coverageReport, sourceConfig);
                        this.setChatContext({
                            topicId: tid,
                            start,
                            end,
                    ...
                    (err) => {
                        ...
                        if (wasAborted && aiText.trim()) {
                            this.updateResultBox(resultBox, aiText, false, coverageReport, failure.sourceConfig);
                        } else {
        ...
        updateResultBox(resultBox, text, isStreaming, coverageReport = null, sourceConfig = null) {
            const Q = this.uiManager.Q.bind(this.uiManager);
            ...
            ` + contentHTML + sourceHTML + coverageHTML;
        ...
        updateBubble(bubbleDiv, text, isStreaming) {
            ...
            if (message) {
                ...
                if (message.status !== 'error') {
                    ...
                    bubbleDiv.innerHTML = this.renderWithThinking(text, isStreaming, isExpanded) + source;
                } else {
...
<body>
    ...
    let activeTopicPrewarmTimer = null;

    const clearActiveTopicPrewarmTimer = () => {
        if (!activeTopicPrewarmTimer) return;
        clearTimeout(activeTopicPrewarmTimer);
    ...
    const scheduleActiveTopicPrewarm = (reason = 'route', delayMs = Core.topicDataPrewarmPolicy.routeDelayMs) => {
        ...
        activeTopicPrewarmTimer = setTimeout(() => {
            activeTopicPrewarmTimer = null;
