 ...
 * 7.8.0-beta.1: 收敛 7.8.0 的兼容性与资源开销：流式回答改用轻量预览和自适应节流，减少历史消息重建与滚动调度；补齐 IME、可视视口、焦点和销毁生命周期；为主题缓存、预热状态与离线图片导出增加字节预算、超时和取消边界。
...
${timePart}${boostPart}${content}`;
  ...
  var topicDataCore = {
    ...
    setCachedTopicData(topicId, topicData, meta = {}) {
      ...
      const confirmedApproxSize = confirmed ? approxSize : Number(previous?.confirmedApproxSize) || (previous?.confirmedTopicData === previous?.topicData ? Number(previous?.approxSize) || 0 : 0);
      const retainedSize = approxSize + (confirmedTopicData && confirmedTopicData !== topicData ? confirmedApproxSize : 0);
  ...
  var exportCore = {
    ...
    async fetchImageAsDataUrl(url, options = {}) {
      ...
      try {
        ...
        if (timedOut) {
          const timeoutError = new Error(`图片请求超过 ${Math.round(timeoutMs / 1e3)} 秒`);
...
` : "\n";
  ...
  var renderingCore = {
    ...
    renderAiOutputHtml(output, options = {}) {
      ...
      const answerHtml = options.isStreaming === true ? this.renderStreamingContentPreview(content) : this.renderMarkdown(content);
      const answer = content.trim() ? `<div class="ai-output-answer" data-selection-scope="answer">${answerHtml}</div>` : "";
  ...
  var style1Helpers = {
    ...
    openModelPicker() {
      ...
      const activeElement = modal?.getRootNode?.().activeElement;
      this.modelPickerReturnFocus = activeElement?.isConnected ? activeElement : Q("#btn-fetch-models");
    ...
    isCurrentExportRequest(request) {
      return Boolean(request && this.activeExportRequest === request && request.token === this.exportRequestSeq && (!request.lifecycleEpoch || this.isCurrentLifecycleEpoch(request.lifecycleEpoch)));
    },
...
<div class="container">
    ...
    <div class="footer">
      ...
      const skippedText = skippedImageResourceCount > 0 ? `<br><small>${skippedImageResourceCount} 个图片资源因失败或资源预算限制保留远程地址</small>` : "";
...
${postContent}` },
    ...
    renderChatMessages() {
      ...
      this.closeSummarySelectionMenu?.();
      list.innerHTML = "";
      this.chatSession.visibleMessages.forEach((message) => this.addBubble(message, void 0, { scroll: false }));
...
${text}` }
  ...
  var style1Lifecycle = {
    ...
    init(uiManager) {
      ...
      this.chatInputMaxHeights = /* @__PURE__ */ new WeakMap();
      this.chatScrollFrameId = null;
      this.chatScrollResetTimerId = null;
      this.chatScrollForce = false;
      this.summaryScrollFrameId = null;
    ...
    getStreamingRenderDelay(output) {
      const content = output && typeof output === "object" ? `${output.reasoningText || ""}${output.contentText || output.content || ""}` : `${output ?? ""}`;
      if (content.length > 32768) return 220;
      if (content.length > 8192) return 140;
    ...
    resizeChatInput(input) {
      ...
      if (!Number.isFinite(maxHeight)) {
        const computedMaxHeight = typeof getComputedStyle === "function" ? Number.parseFloat(getComputedStyle(input).maxHeight) : Number.NaN;
  ...
  var STYLE2_STYLES = `
    ...
    * { box-sizing: border-box; }
    ...
    .sidebar-panel {
        ...
        top: 0;
        bottom: auto;
        height: var(--ui-viewport-height, 100vh);
        max-height: 100vh;
    ...
    .model-picker-overlay.show, .workspace-replace-overlay.show { display: flex; }
    .model-picker-dialog, .workspace-replace-dialog { width: 100%; max-width: 360px; max-height: min(72vh, calc(var(--ui-viewport-height, 100vh) - 40px)); display: flex; flex-direction: column; overflow: hidden; background: var(--ui-surface-raised); border: 1px solid var(--ui-border); border-radius: var(--radius-lg); box-shadow: var(--shadow-menu); }
    ...
    :host(.narrow-viewport) .sidebar-panel { width: min(100vw, var(--ui-viewport-width, 100vw)); max-width: min(100vw, var(--ui-viewport-width, 100vw)); border-right: none; border-left: none; }
    ...
    :host(.narrow-viewport) .model-picker-dialog { max-width: none; max-height: min(82vh, calc(var(--ui-viewport-height, 100vh) - 24px), 680px); border-radius: var(--radius-lg); }
    ...
    @media (max-width: 700px) {
        .sidebar-panel { width: min(100vw, var(--ui-viewport-width, 100vw)); max-width: min(100vw, var(--ui-viewport-width, 100vw)); }
