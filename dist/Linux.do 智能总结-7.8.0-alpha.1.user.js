 ...
 * 7.8.0-alpha.1: 将脚本迁移为多文件源码与确定性单文件构建；按 AI、Discourse、内容、导出和 UI 职责拆分模块，并建立源码测试、生成物校验与 CI 发布门禁。
...
${postContent}` },
    ...
    renderChatErrorContent(message) {
      ...
      const title = formatted?.title || (message.errorKind === "thinking_only" ? "AI 只返回了推理内容" : message.errorKind === "empty_response" ? "AI 返回了空内容" : "AI 回复失败");
