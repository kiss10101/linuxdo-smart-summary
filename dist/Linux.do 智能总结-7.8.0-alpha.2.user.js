 ...
 * 7.8.0-alpha.2: 精简总结选区操作，为生成中与已完成消息提供状态化操作菜单和安全的停止、重新生成、删除流程；重设计默认 Style2 并补齐键盘操作、减少动画和窄屏回退。
...
${postContent}` },
    ...
    isSummarySelectionRangeAllowed(range, resultBox) {
      ...
      return !excludedSelectors.some((selector) => this.getClosestElement(startNode, selector) || this.getClosestElement(endNode, selector) || this.getClosestElement(commonNode, selector));
    },
    ...
    openSummarySelectionMenu(selectionState) {
      ...
      this.currentSummarySelectionReturnFocus = this.uiManager.shadow?.activeElement || this.uiManager.Q("#summary-result");
...
${text}` }
  ...
  var style1State = {
    ...
    isCurrentChatRequest(request) {
      return Boolean(request && this.activeChatRequest === request && request.token === this.chatRequestSeq && !request.abortReason);
    },
