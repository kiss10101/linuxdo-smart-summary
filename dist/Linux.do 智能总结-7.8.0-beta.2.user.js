 ...
 * 7.8.0-beta.2: 修复 AI 流式输出期间先显示原始 Markdown 标记、完成后才渲染的问题；最终答案恢复按节流批次实时解析并清理 Markdown，继续保留长输出自适应节流、推理区纯文本安全预览和最终同步刷新。
 * 7.8.0-beta.1: 收敛 7.8.0 的兼容性与资源开销：减少历史消息重建与滚动调度；补齐 IME、可视视口、焦点和销毁生命周期；为主题缓存、预热状态与离线图片导出增加字节预算、超时和取消边界。
